...
class ServerUtils {
  ...
  */
  static apiNotFoundHandler(req, res) {
    ...
    res.status(ServerUtils.StatusCodes.Error.Missing).sendError(message);
  }
  /**
  ...
  * @param {ServerResponse} res
  */
  static genericErrorHandler(error, req, res) {
    if(error instanceof Error) {
      this.log('error', this.getConfig('logStackOnError') ? error.stack : error.toString());
    } else {
      this.log('error', error);
    }
    res.status(error.statusCode || ServerUtils.StatusCodes.Error.Default).sendError(error);
  }
  /**
