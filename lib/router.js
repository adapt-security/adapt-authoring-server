    ...
    if(this.routes.length) {
      this.routes.forEach(r => {
        Object.entries(r.handlers).forEach(([method, handler]) => {
          ...
          this.expressRouter[method](r.route, this.extendRequestData, this.invokeRequestHook, handler);
        });
      });
    }
  ...
  */
  extendRequestData(req, res, next) {
    function sendError(error) {
      ...
      this.status(error.statusCode || res.StatusCodes.Error.Default).json({ message });
    }
