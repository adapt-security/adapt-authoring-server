...
class Router {
    ...
    if(this.routes.length) {
      this.routes.forEach(r => {
        Object.entries(r.handlers).forEach(([method, handler]) => {
          ...
          this.expressRouter[method](r.route, this.extendRequestData.bind(this), this.invokeRequestHook, handler);
        });
      });
    }
  ...
  */
  extendRequestData(req, res, next) {
    const sendError = (statusCode, error = this.t('error.genericrequesterror')) => {
      if(!_.isNumber(statusCode)) {
        return this.log('error', this.t('error.invalidstatuscode'));
      }
      res.status(statusCode || res.StatusCodes.Error.Default);

      if(_.isString(error)) {
        return res.json({ message: error });
      }
      if(!error.hasOwnProperty('message')) {
        return this.log('error', this.t('error.invaliderrorformat'));
      }
      res.json(error);
    };
