...
class Router {
    ...
    if(this.routes.length) {
      this.routes.forEach(r => {
        Object.entries(r.handlers).forEach(([method, handler]) => {
          const route = `${r.route}${r.param ? ':' + r.param : ''}`;
          this.log('debug', App.instance.lang.t('info.addroute', { method: method.toUpperCase(), route: this.path+route }));
          this._router[method](route, this.handleRequest(), handler);
        });
      });
    }
  ...
  */
  genericNotFoundHandler(req, res, next) {
    new Responder(res).error(App.instance.lang.t('error.notfound', { method: req.method, url: req.originalUrl }, { statusCode: 404 }));
  }
  /**
