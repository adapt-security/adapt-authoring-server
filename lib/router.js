...
class Router {
  ...
  * @param {Object} r Route config
  * @return {Boolean}
  */
  validateRoute(r) {
    const ePrefix = App.instance.lang.t('error.invalidroute', { path: `${this.path}/${r.route}` });
    if(!Utils.isString(r.route)) {
      this.log('warn', `${ePrefix}, ${App.instance.lang.t('error.routenotstring')}`);
      return false;
    }
    if(!r.handlers) {
      this.log('warn', `${ePrefix}, ${App.instance.lang.t('error.nohandlers')}`);
      return false;
    }
    if(!Object.values(r.handlers).every(Utils.isFunction)) {
      this.log('warn', `${ePrefix}, ${App.instance.lang.t('error.handlersnotfuncs')}`);
      return false;
    }
  }
  /**
