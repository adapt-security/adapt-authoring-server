...
class Router {
  ...
  */
  get map() {
    const toMapObj = r => { return {  [`${r.route}_url`]: r.url  } };
    return this.childRouters
      .sort((a,b) => a.route.localeCompare(b.route))
      .reduce((m,c) => Object.assign(m, toMapObj(c)), toMapObj(this));
  }
  /**
  ...
  */
  enableAPIMap() {
    if(this.checkHasInited(`${this.t('error.routeralreadyinited')}, ${this.t('error.noenablemap')} (${this.path})`)) {
      return;
    }
    this.addRoute({
      route: '/',
