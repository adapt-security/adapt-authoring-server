...
class ServerUtils {
  ...
  static genericErrorHandler(error, req, res, next) {
    this.log('error', this.getConfig('logStackOnError') && error.stack ? error : error.toString());
    res.sendError(error);
  }
  /**
  ...
  static addErrorHandler(req, res, next) {
    res.sendError = error => {
      ...
      res
        ...
        .json({ 
          code: error.code, 
          message: req.translate(`error.${error.code}`, error.data)
        });
    };
    next();
  }
   ...
   * @param {Function} next
   */
  static handleInternalRoutes(req, res, next) {
    if(req.routeConfig.internal && req.hostname !== 'localhost') {
      return next(this.app.errors.UNAUTHORISED.setData({ url: req.originalUrl, method: req.method }));
    } 
    next();
  }
  /**
