...
class ServerUtils {
  ...
  static methodNotAllowedHandler (apiRouter) {
    return (req, res, next) => {
      // Get all registered routes and their methods
      ...
      for (const [path, methods] of routeMap.entries()) {
        // Compare the requested path with registered routes
        // We need to handle both exact matches and parameterized routes
