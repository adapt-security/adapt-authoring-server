...
class ServerUtils {
  ...
  /**
   * Middleware for handling 405 errors on the API router
   * @param {external:ExpressRequest} req
   * @param {external:ExpressResponse} res
   * @param {Function} next
   */
  static apiInvalidMethodHandler (req, res, next) {
    next(App.instance.errors.INVALID_METHOD.setData({ endpoint: req.originalUrl, method: req.method }))
  }
