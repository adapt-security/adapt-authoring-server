...
class ServerUtils {
  ...
  /**
   * Middleware for handling 405 Method Not Allowed errors on the API router
   * Checks if the requested path exists with a different HTTP method
   * @param {Router} apiRouter The API router to check routes against
   * @return {Function} Middleware function
   */
  static methodNotAllowedHandler (apiRouter) {
    // Pre-compute route patterns with their methods
    // This is done once when the handler is created, not on every request
    const routePatterns = []
    const routeMap = apiRouter.getAllRoutes()

    for (const [path, methods] of routeMap.entries()) {
      // Replace Express-style params with regex pattern
      // Handle optional parameters by making the segment (including slash) optional
    ...
    return (req, res, next) => {
      ...
      // Check if the requested path matches any registered route
      for (const { regex, methods } of routePatterns) {
        if (regex.test(req.path)) {
          // Path matches - check if method is allowed
          ...
          // Method is allowed, route handler will take care of it
          return next()
        }
      }

      // No matching route found, continue to 404 handler
      next()
    }
  }
