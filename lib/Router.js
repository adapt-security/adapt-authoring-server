...
class Router {
  ...
  constructor(route, parentRouter) {
    ...
    this.routerMiddleware = [
      ServerUtils.addTranslationUtils,
    ...
    if(this.routes.length) {
      this.routes.forEach(r => {
        Object.entries(r.handlers).forEach(([method, handler]) => {
          this.log('debug', `added ${method.toUpperCase()} ${this.path !== '/' ? this.path : ''}${r.route}`);
          this.expressRouter[method](r.route, ...this.getHandlerMiddleware(), handler);
        });
      });
    }
