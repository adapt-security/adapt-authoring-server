...
class Router {
   ...
   * @param {Array<Function>} handlerMiddleware Array of middleware for route handlers
   */
  constructor (root, parentRouter, routes = [], middleware = [], handlerMiddleware = []) {
    /**
     ...
     * @type {Array<Route>}
     */
    this.routes = routes.filter(this.validateRoute, this)
    /**
     * Express router instance
    ...
    this.routerMiddleware = [
      ...
      ...middleware.filter(_.isFunction)
    ]
    /**
   ...
   */
  validateRoute (r) {
    const ePrefix = `invalid route config for ${this.root} router`
   ...
   * @param {Array<function>} middleware Array of middleware to add
   * @param {Array<function>} handlerMiddleware Array of middleware to add to route handlers
   * @return {Router} The new router instance
   */
  createChildRouter (root, routes, middleware, handlerMiddleware) {
    if (this.warnOnInited(`cannot create further child routers (${this.path}/${root})`)) {
      return this
    }
    const router = new Router(root, this, routes, middleware, handlerMiddleware)
  ...
  getAllRoutes () {
    ...
    this.flattenRouters().forEach(router => {
      router.routes.forEach(route => {
        // Construct the full path for this route
        const fullPath = `${router.path !== '/' ? router.path : ''}${route.route}`

        // Get existing methods for this path or create a new set
        if (!routeMap.has(fullPath)) {
          routeMap.set(fullPath, new Set())
        }

        // Add all methods for this route
